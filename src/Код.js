...
const BOND_HEADERS = [
    ...
    'Quantity',
    'Purchase Price (UAH)',
    'Accrued Interest at Purchase (UAH)',
    'Interest Rate (%)',
    'Tax Rate (%)',
    'Currency',
    'Purchase Date',
...
function setupSheet() {
    ...
    const configSheet = createOrGetSheet(
        ss,
        SHEET_NAMES.CONFIG,
...
function generateCouponSchedule(input) {
    ...
    const standardPeriodDays =
        freq === 6 ? 182.5 : freq === 3 ? 91.25 : freq === 12 ? 365 : 30.4;
    ...
    for (let i = 0; i < allCouponDates.length; i++) {
        ...
        const periodStart =
            i > 0
                ? allCouponDates[i - 1]
                : inferPeriodStartBeforeFirst(allCouponDates[0], freq);
        ...
        const holderPeriodStart =
            coupons.length === 0 ? input.purchaseDate : periodStart;
        ...
        if (input.fixedCouponPerUnit > 0) {
            ...
            grossAmount =
                bankersRound(input.fixedCouponPerUnit * input.quantity * 100) /
                100;
        } else {
            // CALCULATED MODE
...
function refreshAnalytics() {
    ...
    const metrics = [
        ...
        [
            ...
            formatUAH(r(totalFaceValue - totalInvested)),
        ],
        ...
        [
            ...
            formatUAH(r(totalGrossCouponIncome)),
        ],
        [
            'Total Tax Paid',
            formatUAH(r(totalGrossCouponIncome - totalNetCouponIncome)),
        ],
        [
            'Total Net Coupon Income Received',
            formatUAH(r(totalNetCouponIncome)),
        ],
        ['', ''],
        ['── INCOME (PROJECTED) ──', ''],
...
function refreshLadder(ladderSheet, activeBonds, today) {
    if (ladderSheet.getLastRow() > 1) {
        ladderSheet
            .getRange(2, 1, ladderSheet.getLastRow() - 1, LADDER_HEADERS.length)
            .clearContent();
    }
    ...
    const rows = buckets.map((b) => {
        ...
        const pct =
            totalFace > 0
                ? bankersRound((bucketFace / totalFace) * 10000) / 100
                : 0;
        return [
            b.label,
            b.bonds.length,
...
function buildCashflows(bonds, couponsData) {
    ...
    bonds.forEach((bond) => {
        ...
        if (
            normalizeDate(bond.maturityDate) >= today ||
            bond.status !== 'SOLD'
        ) {
            flows.push({
